...
export default function HelpList({ type, q }: IHelpList) {
    ...
    return q
      ? posts.filter((e) => {
          return (
            ...
            e.tags.some((elem) => elem.toLowerCase().includes(q.toLowerCase()))
          );
        })
      : posts;
  }, [posts, q]);
    ...
    switch (type) {
      case PostTypes.DECLARED:
        ...
        postsToSet = myPosts.docs
          .map((e: DocumentData) => ({ ...e.data(), uuid: e.id } as IPost))
          .sort((a: IPost, b: IPost) => {
            return (
              ...
              calculateDistance(b.address as ILocation, myAddress as ILocation)
            );
          });
        break;
      case PostTypes.MINE:
  ...
  if (anyError) {
    console.error(error, error2, addressError);
    return (
      <ErrorPage
        name={"Nieznany błąd"}
        ...
        (filteredPosts && filteredPosts.length > 0 ? (
          <>
            <Flex
              ...
              // alignItems={"center"}
            >
              {filteredPosts.filter((e) => !e.isCompleted).length === 0 ? (
            ...
            {(type === PostTypes.MINE || type === PostTypes.DECLARED) && (
              <>
